...
if __name__ == '__main__':
	...
	attack = 'Singleton_d'  # IncBP, IncDS, IncPR, Random, Singleton, demotion Random attacker,demotion Singleton attacker에 한해 Random_d,Singleton_d
  ...
  #demotion을 위한 singleton accounts 생성, target은 동일함
	elif attack == 'Singleton_d':
		added_edges = []
		for user in user_product_graph.keys():
			new_id = int(user) + 1
		c = [str(user) for user in range(new_id+targets*r, new_id + 2*targets*r)]
		for index, prod in enumerate(t):
			for account in c[r * index:r * (index+1)]:
				added_edges.append((account, prod))
