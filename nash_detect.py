...
def run_detectors(upg, pug, added_reviews_p, added_reviews_d, detectors_q, top_k_per):
	"""
...
def compute_cost(ori, new, added_reviews, filter_reviews, target_ids, elite_accounts):
	...
	for review in remain_reviews:
		...
		#	cost[review] = 0
		if review in elite_reviews:
			cost[review] = abs(0.035*(new_RI[item]-ori_RI[item])/item_review_count[item]\
...
def compute_reward(ori, new, target_ids, mode):
	"""
	...
	for item in target_ids:
		...
		# Eq. (3) in paper 변형했다.
		min_PE = PE if PE <= min_PE else min_PE
	...
	for item, PE in PE_dict.items():
		if mode==5:#promotion일때 reward 계산식
