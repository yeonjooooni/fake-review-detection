...
def testing_detectors(upg, pug, added_reviews_p, added_reviews_d, detectors_q, top_k_per, detector='GANG'):
	"""
	...
	spammer_ids = []
	for review in added_reviews_p:
		if review[0] not in spammer_ids:
			spammer_ids.append(review[0])

	for review in added_reviews_d:
		if review[0] not in spammer_ids:
			spammer_ids.append(review[0])

	u_p_graph, p_u_graph = cp.deepcopy(upg), cp.deepcopy(pug)

	new_priors, u_p_graph, p_u_graph, user_ground_truth_p, review_ground_truth_p, new_added_reviews_p = add_adversarial_review(u_p_graph, p_u_graph, added_reviews_p, 5)
	new_priors, u_p_graph, p_u_graph, user_ground_truth_d, review_ground_truth_d, new_added_reviews_d = add_adversarial_review(u_p_graph, p_u_graph, added_reviews_d, 1)
	...
	new_added_reviews = new_added_reviews_p+new_added_reviews_d
	...
	for review in top_k_reviews:
		if review not in added_reviews_p and review not in added_reviews_d:
...
if __name__ == '__main__':
	...
	testing_detector = 'Nash-Detect'  # Equal-Weights, Nash-Detect, Fraudar, SpEagle, GANG, Prior, fBox,
	...
	setting6 = 'Testing/' + dataset_name + '/Random_d.pickle'
	setting7 = 'Testing/' + dataset_name + '/Singleton_d.pickle'
	...
	# load fake reviews for promotion attack to each target item
	item_attack_review = load_fake_reviews(attacks, targets, paths_p)
	
  # load fake reviews for demotion attack
	...
	attacks_p = {}
	attacks_p_d = {}
	optimal_q = {'YelpChi': {'GANG': 1.45, 'Prior': 0.78, 'SpEagle': 0.23, 'fBox': 0.44, 'Fraudar': 1.65},
	...
	attack_d_log = {a: [] for a in attacks_p_d.keys()}
	pe_log, return_log = [], []
	init_time = time.time()
	...
	for i in range(iters):
		...
		r_spam_beliefs, top_k_reviews, new_prod_user_graph = testing_detectors(user_product_graph, prod_user_graph, promotion_posted_reviews[i], demotion_posted_reviews[i], detectors_q, top_k, detector=testing_detector)
